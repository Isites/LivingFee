...
public class TextParser {
    ...
    public static Spanned fromHtml(String source,int width, Context context){
        return Html.fromHtml(source, new UriImageGetter(width, context), new TagHandler());
    }
    ...
    private static void withinParagraph(StringBuilder out, Spanned text, int start, int end, int nl) {
        ...
        for (int i = start; i < end; i = next) {
            ...
            for (int j = 0; j < spans.length; j++) {
                ...
                if (spans[j] instanceof ImageStyle) {
                    out.append("<img src=\"");
                    out.append(((ImageStyle) spans[j]).getPath());
                    out.append("\">");
